...
export class SetsService {
  ...
  async create(createSetDto: CreateSetDto, userId: string) {
    const user = await this.userRepository.findOne({ where: { id: userId } });
    if (!user) throw new HttpException('User not found', HttpStatus.NOT_FOUND);
    const exercise = await this.exerciseRepository.findOne({
      where: { id: createSetDto.exerciseId },
      relations: ['user'],
    });
    if (!exercise)
      throw new HttpException('Exercise not found', HttpStatus.NOT_FOUND);
    if (exercise.user.id != userId)
      throw new HttpException('Incorrect exercise', HttpStatus.BAD_REQUEST);
  ...
  async findOne(id: number, firebaseUID: string) {
    ...
    if (set.user.id != firebaseUID)
      throw new HttpException('Unauthorized action', HttpStatus.UNAUTHORIZED);
    return set;
  }

  async update(id: number, updateSetDto: UpdateSetDto, userId: string) {
    const exists = await this.setRepository.exist({
      where: {
        id,
        user: { id: userId },
      },
    ...
    const result: UpdateResult = await this.setRepository.update(
      id,
      updateSetDto,
    );
    if (result.affected != 1)
      throw new HttpException('Set not updated', HttpStatus.NOT_MODIFIED);
  ...
  async remove(id: number, firebaseUID: string) {
    ...
    if (result.affected != 1)
      throw new HttpException('Set not deleted', HttpStatus.NOT_MODIFIED);
    return { id };
  }

  async findAllByUserIdAndQueries(
    userId: string,
    exerciseId?: number,
    startDate?: Date,
    endDate?: Date,
  ) {
    let filter: FindOptionsWhere<Set> | FindOptionsWhere<Set>[] = {
      user: { id: userId },
    };
    ...
    if (exerciseId) {
      ...
      if (exercise.user.id != userId)
        throw new HttpException('Unauthorized action', HttpStatus.UNAUTHORIZED);
      filter = { ...filter, exercise: { id: exerciseId } };
    }

    if (startDate && endDate) {
      filter = {
        ...filter,
        date_created: Between(startDate, endDate),
      };
    }
